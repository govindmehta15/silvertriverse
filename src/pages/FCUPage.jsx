...
import { fcuFilms, FCU_PHASES, formatPrice, formatLargePrice } from '../data/fcuData';
...
/* ─── Token progress ring ──────────────────────────────────── */
function TokenRing({ sold, total, size = 54 }) {
    ...
    return (
        <svg width={size} height={size} className="shrink-0 -rotate-90">
            ...
            <circle cx={size / 2} cy={size / 2} r={r} fill="none" stroke="rgba(201,162,39,0.7)" strokeWidth="4"
...
export default function FCUPage() {
    ...
    const masterpieces = fcuFilms.filter(f => f.masterpiece).map(f => ({ ...f.masterpiece, filmTitle: f.title, filmId: f.id }));
    const totalMarketCap = fcuFilms.reduce((s, f) => s + f.marketCap, 0);
    const totalTokensSold = fcuFilms.reduce((s, f) => s + f.tokensSold, 0);
    const totalTokens = fcuFilms.reduce((s, f) => s + f.totalTokens, 0);
    const avgChange = (fcuFilms.reduce((s, f) => s + f.priceChange, 0) / fcuFilms.length).toFixed(2);
                    ...
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}
                        ...
                        {FCU_PHASES.map((p, i) => (
                            <div key={p.key} className="flex items-center">
                                ...
                                {i < FCU_PHASES.length - 1 && <span className="text-gold/15 text-[9px] mx-0.5">→</span>}
                            </div>
                        ))}
                    </motion.div>
                </div>
            ...
            <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
                ...
                <div className="rounded-xl border border-navy-700/30 p-5 relative overflow-hidden"
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div>
                            <p className="text-gray-500 text-[9px] uppercase tracking-wider">FCU Index</p>
                        ...
                        <div>
                            ...
                            <p className="text-white font-mono font-bold text-lg">{fcuFilms.length}</p>
                            <p className="text-gray-600 text-[9px]">{fcuFilms.filter(f => f.phase === 'bidding').length} accepting bids</p>
                        </div>
                    </div>
                    {/* Animated chart line */}
                ...
                {filtered.map(film => {
                    ...
                    return (
                        <motion.div key={film.id} variants={item}
                            onClick={() => navigate(`/fcu/film/${film.id}`)}
            ...
            {masterpieces.length > 0 && (
                <div className="px-4 max-w-4xl mx-auto mb-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {masterpieces.map(mp => (
                            <MasterpieceCard key={mp.id} mp={mp} filmTitle={mp.filmTitle} onClick={() => navigate(`/fcu/film/${mp.filmId}`)} />
                        ))}
                    </div>
                </div>
            )}
