...
export default function ReelityFeedPage() {
    ...
    return (
        <div className="space-y-6">
                ...
                {interleavedFeed.map((feedItem, i) => {
                    ...
                    return (
                        <motion.div key={feedItem.id} variants={item} className="bg-navy-900/60 border border-navy-700/50 rounded-xl overflow-hidden">
                            ...
                            {feedItem.image && <img src={feedItem.image} alt="" className="w-full aspect-video object-cover" />}
                            <div className="p-4 pt-3 space-y-3">
                                <p className="text-gray-200 text-sm">{feedItem.caption}</p>
                                <div className="flex items-center gap-4">
                                    <button onClick={() => user && likeMutation.mutate({ postId: feedItem.id, userId: user.id })} className={`flex items-center gap-1.5 text-xs transition-colors ${feedItem.likes?.includes(user?.id) ? 'text-red-400' : 'text-gray-400 hover:text-red-400'}`}>
                                        <span>{feedItem.likes?.includes(user?.id) ? '❤️' : '🤍'}</span>
                                        <span>{feedItem.likes?.length || 0}</span>
                                    </button>
                                    <span className="text-gray-400 text-xs flex items-center gap-1.5">💬 {feedItem.comments?.length || 0}</span>
                                </div>
                                        ...
                                        <input
                                            ...
                                            onKeyDown={e => {
                                                if (e.key === 'Enter' && commentText[feedItem.id]?.trim()) {
                                                    commentMutation.mutate({ postId: feedItem.id, commentData: { authorId: user.id, authorName: user.name, text: commentText[feedItem.id] } });
                                                }
                                            }}
            ...
            {battle && (
                <section className="pt-8 border-t border-navy-600/30">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative">
                        <div className="hidden md:flex absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-20 w-12 h-12 rounded-full bg-navy-900 border-2 border-gold/40 items-center justify-center text-gold font-bold font-serif shadow-[0_0_20px_rgba(201,162,39,0.3)]">VS</div>
                        {battle.films.map((film) => (
                            <motion.div key={film.id} whileHover={{ y: -5 }} className={`relative overflow-hidden rounded-2xl border transition-all duration-300 ${votedFor === film.id ? 'border-gold shadow-glow-gold bg-gold/5' : 'border-navy-600/40 bg-navy-800/60'} ${hasVoted && votedFor !== film.id ? 'opacity-50 grayscale-[50%]' : ''}`}>
                                ...
                                <div className="relative p-6 flex flex-col h-full min-h-[250px]">
                                    {votedFor === film.id && <div className="absolute top-4 right-4 bg-gold text-navy-900 text-xs font-bold px-3 py-1 rounded-full shadow-md">✓ Your Pick</div>}
                                    <div className="mt-auto">
                                        ...
                                        <button onClick={() => !hasVoted && voteMutation.mutate(film.id)} disabled={hasVoted} className={`w-full py-3 rounded-lg font-bold text-sm uppercase tracking-wide transition-all ${hasVoted ? (votedFor === film.id ? 'bg-gold text-navy-900' : 'bg-navy-700 text-gray-500') : 'bg-white/10 text-white hover:bg-white/20 backdrop-blur-sm border border-white/20'}`}>
            ...
            {/* Post Form Modal */}
            {showPostForm && (
                <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setShowPostForm(false)}>
                    <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} onClick={e => e.stopPropagation()} className="bg-navy-900 border border-navy-700 rounded-2xl p-6 w-full max-w-md space-y-4">
                        ...
                        <textarea value={postCaption} onChange={e => setPostCaption(e.target.value)} placeholder="What's happening in your creative world? 🎬" rows={4} className="w-full bg-navy-800 border border-navy-600 rounded-lg p-3 text-sm text-white placeholder-gray-500 focus:border-gold/50 focus:outline-none resize-none" />
                        <div className="flex gap-3">
                            <button onClick={() => setShowPostForm(false)} className="flex-1 py-2.5 rounded-lg bg-navy-800 text-gray-400 text-sm font-medium hover:bg-navy-700 transition-colors">Cancel</button>
                            <button onClick={() => { if (!postCaption.trim()) return; postMutation.mutate({ authorId: user.id, authorName: user.name, authorAvatar: user.avatar, caption: postCaption, image: user.avatar }); }} disabled={postMutation.isPending || !postCaption.trim()} className="flex-1 py-2.5 rounded-lg bg-gold text-navy-950 text-sm font-bold hover:bg-gold/90 transition-colors disabled:opacity-50">
